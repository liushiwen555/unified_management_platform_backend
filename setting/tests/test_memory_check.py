...
from setting.system_check import MemoryCheck
...
class TestMemoryCheck:
    ...
    def clean_cache(self):
        rs.delete(MemoryCheck.exceed_key)
        rs.delete(MemoryCheck.alert_key)
        return None

    def test_add_exceed_count(self, clean_cache):
        cpu_check = MemoryCheck(10)

        cpu_check.add_exceed_count()
        assert int(rs.get(MemoryCheck.exceed_key)) == 1

        for _ in range(10):
            cpu_check.add_exceed_count()
        assert int(rs.get(MemoryCheck.exceed_key)) == 11
    ...
    def test_generate_alarm(self, clean_cache):
        ...
        assert UnifiedForumLog.objects.filter(
            content__contains='内存使用率').exists()
    ...
    def test_check_with_alarm(self, clean_cache):
        cpu_check = MemoryCheck(-1)
        rs.set(MemoryCheck.exceed_key, 20)
        cpu_check.check()
    ...
    def test_check_without_alarm(self, clean_cache):
        cpu_check = MemoryCheck(100)
        rs.set(MemoryCheck.exceed_key, 20)
        cpu_check.check()

        assert not UnifiedForumLog.objects.filter(
            content__contains='内存使用率').exists()
        assert not MemoryEvent.get_queryset(content__contains='内存使用率').exists()
    ...
    def test_check_complex(self, clean_cache):
        ...
        """
        cpu_check = MemoryCheck(-1)
        rs.set(MemoryCheck.exceed_key, 10)
        # 1. 产生了告警
        cpu_check.set_alert_status()

        # 2. 告警期间内不统计exceed_count
        cpu_check.check()
        assert not rs.get(MemoryCheck.exceed_key)
